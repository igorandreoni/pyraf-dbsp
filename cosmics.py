...
class cosmicsimage:
	...
	def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0, sigclip = 5.0, sigfrac = 0.3, objlim = 5.0, satlevel = 50000.0, verbose=True, objectOrder=0, skyOrder=0):
		"""
		...
		self.skyOrder = skyOrder
		self.objectOrder = objectOrder
		self.spectralMode = (skyOrder > 0) or (objectOrder > 0)
        	...
        	self.objectTrace = None # only used for spectral
        	self.skyLines = None
	...
	def subtractObjectSpectrum(self, verbose=None):
		"""
		...
		if verbose:
			print "Subtracting object traces using order of %i..."%self.objectOrder
		
		self.objectTrace = np.zeros(self.cleanarray.shape)
		
		transpose = self.cleanarray.shape[0] > self.cleanarray.shape[1]
		
		if transpose:
			self.cleanarray = self.cleanarray.transpose()
			self.objectTrace = self.objectTrace.transpose()
		
		for i in range(self.cleanarray.shape[0]):
			self.objectTrace[i] = fit1dleg(self.cleanarray[i], order=self.objectOrder, low=4, high=4, niter=3)

		self.cleanarray -= self.objectTrace
	...
	def subtractSkyLines(self, verbose=None):
		...
		if verbose:
			print "Subtracting sky lines using order of %i..."%self.objectOrder
		
		self.skyLines = np.zeros(self.cleanarray.shape)
		...
		for i in range(self.cleanarray.shape[0]):
			self.skyLines[i] = fit1dleg(self.cleanarray[i], order=self.skyOrder, low=4, high=4, niter=3)

		self.cleanarray -= self.skyLines
	...
	def run(self, maxiter = 4, verbose = False):
		...
		# see if we need to subtract spectral or imagining features
		if self.spectralMode:
			if self.objectOrder > 0 and self.objectTrace is None:
				self.subtractObjectSpectrum()
			if self.skyOrder > 0 and self.skyLines is None:
				self.subtractSkyLines()
		# otherwise imaging mode
		elif self.satlevel > 0 and self.satstars == None:
		...
		if self.objectTrace is not None:
			self.cleanarray += self.objectTrace
		if self.skyLines is not None:
			self.cleanarray += self.skyLines
...
def fit1dleg(line, order=3, low=4, high=4, niter=3):
	...
	while niter > 0:
		...
		spl = interpolate.LSQUnivariateSpline(xSub, ySub, np.arange(0, xSub.shape[0], int(xSub.shape[0]/(order+1)))[:-1]+int(0.5*xSub.shape[0]/(order+1)))
		yZero = spl(xSub)-ySub
